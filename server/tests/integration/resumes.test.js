...
describe("POST /api/resumes", () => {
  it("creates a resume defaulting to domain=swe, templateId=jakes", async () => {
    ...
    expect(res.body.domain).toBe("swe");
    expect(res.body.templateId).toBe("jakes");
  });
  ...
  it("respects an explicit domain/templateId", async () => {
    ...
    expect(res.body.domain).toBe("cybersecurity");
    expect(res.body.templateId).toBe("infosec");
  });
...
describe("PATCH /api/resumes/:id", () => {
  it("updates domain and templateId independently", async () => {
    ...
    const res = await request(app)
      .patch(`/api/resumes/${create.body._id}`)
      .set("Cookie", authCookie)
      .send({ domain: "devops", templateId: "devops-sre" });
    expect(res.status).toBe(200);
    expect(res.body.domain).toBe("devops");
    expect(res.body.templateId).toBe("devops-sre");
  });
...
describe("POST /api/resumes/:id/duplicate", () => {
  it("carries over domain and templateId to the duplicate", async () => {
    ...
    const dup = await request(app).post(`/api/resumes/${create.body._id}/duplicate`).set("Cookie", authCookie);
    expect(dup.status).toBe(201);
    expect(dup.body.domain).toBe("ai-ml");
